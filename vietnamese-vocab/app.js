...
document.addEventListener("DOMContentLoaded", async () => {
  ...
  try {
    ...
    state.statuses = readJson(STORAGE.status, {});
...
function setStatus(word, status) {
  ...
  window.VocabSync?.recordStatus(word.key);
}
...
function saveDailySession(session) {
  ...
  if (localStorage.getItem(STORAGE.daily) === serialized) return;
  localStorage.setItem(STORAGE.daily, serialized);
  window.VocabSync?.recordDaily();
}
...
function renderStudy() {
  ...
  if (!isDaily) saveLastWord(word.key);
  if (isDaily) {
    const daily = getDailySession();
...
function setWrongKeys(keys) {
  const previous = getWrongKeys();
  const normalized = [...new Set(keys.map(String))];
  localStorage.setItem(STORAGE.wrong, JSON.stringify(normalized));
  window.VocabSync?.recordWrongKeys(previous, normalized);
}

function saveLastWord(wordKey) {
  const key = String(wordKey);
  if (localStorage.getItem(STORAGE.lastWord) === key) return;
  localStorage.setItem(STORAGE.lastWord, key);
  window.VocabSync?.recordLastWord();
}
...
window.addEventListener("vocab-sync-applied", () => {
  state.statuses = readJson(STORAGE.status, {});
